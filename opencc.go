...
func (cc *OpenCC) initDict() error {
	...
	configFile := cc.join(FILES_DIR, configDir, cc.Conversion+".json")
	rc, err := files.Open(configFile)
	if err != nil {
		return err
	}
...
func (cc *OpenCC) addDictChain(d map[string]interface{}) (*Group, error) {
	...
	if typ, ok := t.(string); ok {
		...
		case "txt":
			...
			filename := cc.join(FILES_DIR, dictDir, file.(string))
			rc, err := files.Open(filename)
			if err != nil {
				return nil, err
			}
